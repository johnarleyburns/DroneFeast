    ...
    public class BuildPostprocessor
    {
        ...
        public static void OnPostprocessBuild(BuildTarget target, string pathToBuiltProject)
        {
            if (target == BuildTarget.StandaloneWindows || target == BuildTarget.StandaloneWindows64)
            {
                ...
                {
                    FileUtil.CopyFileOrDirectory(Application.dataPath + Util.Constants.TTS_WINDOWS_EDITOR_x86, dataPath + "RTVoiceTTSWrapper.exe");
                }
                else
                {
                    FileUtil.CopyFileOrDirectory(Application.dataPath + Util.Constants.TTS_WINDOWS_EDITOR, dataPath + "RTVoiceTTSWrapper.exe");
                }
